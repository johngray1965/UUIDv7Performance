...
import io.legere.uuidv7.UUIDv7r01.lastMillis
import io.legere.uuidv7.UUIDv7r01.lastSeq12
...
//    private val numberGenerator: Random = ThreadLocalRandom()
    ...
    private fun randomBytesMonotonic(): ByteArray {
        val value = ByteArray(16).also {  ThreadLocalRandom.current().nextBytes(it) }
    ...
    private fun nextSeq(ts: Long): Int {
        while (true) {
            ...
            if (ts != lastTs) {
                ...
                val seeded =  ThreadLocalRandom.current().nextInt(1 shl 12)
